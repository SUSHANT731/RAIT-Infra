...
<style>
    ...
    * {
    ...
    body {
        background-image: linear-gradient(rgba(243, 198, 194, 0.329),
                rgba(243, 198, 194, 0.329)),
            url("");
    }
    ...
    <section class="main-content">
        ...
        <div class="demo">
            ...
            try {
                ...
                $stmt = $pdo->prepare("select lab,id,po_no,d_name,srno,d_config,infra.dept,infra.status,infra.tags_id,cost,count(d_config) as quant,sum(cost) as totalsum  from infra join allot join istatus join tags join deprt join location on 
                infra.allot = allot.allot and infra.status = istatus.status and infra.tags_id = tags.tags_id and
                infra.dept = deprt.dept and allot.lid = location.lid where lab=:l group by d_config");
